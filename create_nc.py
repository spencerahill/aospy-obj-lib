...
from aospy import regions

# Specify control and perturbation SST file parameters.
nc_cont_path = ('/archive/yim/sm2.1_fixed/SM2.1U_Control-1860_lm2_aie_rerun6'
                '.YIM/pp/ocean/ts/monthly/20yr/ocean.006101-008012.SST.nc')
nc_pert_path = ('/archive/yim/sm2.1_fixed/SM2.1U_Control-1860_lm2_aie2_rerun6'
                '.YIM/pp/ocean/ts/monthly/20yr/ocean.006101-008012.SST.nc')
sst_cont_name = 'SST'
sst_pert_name = 'SST'
lat_cont_name = 'yto'
...
# Load the control lat, lon and SST data and perturbation SST data.
nc_cont = Dataset(nc_cont_path, 'r')
nc_pert = Dataset(nc_pert_path, 'r')
lat_cont = nc_cont.variables[lat_cont_name][:]
lon_cont = nc_cont.variables[lon_cont_name][:]
sst_cont = nc_cont.variables[sst_cont_name][:]
sst_pert = nc_pert.variables[sst_pert_name][:]

# Subtract to get an anomaly field.
anom = 2
print "Imposed SST anomaly:", anom
if anom == 'pert_minus_cont':
    sst_anom_in = sst_pert - sst_cont
elif type(anom) in (int, float):
    sst_anom_in = np.zeros(sst_pert.shape) + anom
else:
    sst_anom_in = np.zeros(sst_pert.shape)


# "Pivot" the longitudes to span 0 to 360 degrees.
def get_pivot_index(longitudes):
    """Get index where longitudes change sign."""
    return np.where(np.diff(np.sign(longitudes)))[0][0] + 1


def pivot_data(data, pivot_index, axis=None):
    """
    Pivot data so that its longitude ranges from 0 to 360.
    Designed to mimic the 'lonPivot' function of NCL.
    """
    return np.roll(data, -pivot_index, axis=axis)
...
# Set names of needed netCDF attributes based on which model the SSTs are for.
model = 'hiram'
print "Model:", model
if model == 'am2':
    nc_clim_path = '/net/yim/sst/reyoi_sst.data.nc'
    time_dim_clim_name = 'time'
    lat_dim_clim_name = 'lat'
    lon_dim_clim_name = 'lon'
    time_clim_name = 'time'
    lat_clim_name = 'lat'
    lon_clim_name = 'lon'
    sst_clim_name = 'sst'
    num_records_clim_name = 'nrecords'
    year_clim_name = 'yr'
    month_clim_name = 'mo'
    day_clim_name = 'dy'
    idim_clim_name = 'idim'
    name_out_prefix = 'reyoi/reyoi_sst'
elif model == 'am3':
    nc_clim_path = '/net/yim/sst/sst.climo.1981-2000.data.nc'
    time_dim_clim_name = 'TIME'
    lat_dim_clim_name = 'LATITUDE'
    lon_dim_clim_name = 'LONGITUDE'
    time_clim_name = 'TIME'
    lat_clim_name = 'LATITUDE'
    lon_clim_name = 'LONGITUDE'
    sst_clim_name = 'SST'
    num_records_clim_name = 'nrecords'
    year_clim_name = 'yr'
    month_clim_name = 'mo'
    day_clim_name = 'dy'
    T1_clim_name = 'average_T1'
    T2_clim_name = 'average_T2'
    DT_clim_name = 'average_DT'
    idim_clim_name = 'idim'
    name_out_prefix = 'hurrell/hurrell_sst.clim'
    fill_value = -1.e34
elif model == 'hiram':
    nc_clim_path = '/net/yim/sst/HadISST_sst.nc'
    time_dim_clim_name = 'time'
    lat_dim_clim_name = 'lat'
    lon_dim_clim_name = 'lon'
    time_clim_name = 'time'
    ...
    day_clim_name = 'dy'
    idim_clim_name = 'idim'
    name_out_prefix = 'HadISST/HadISST_sst.clim'
print "Climatological SST source:", nc_clim_path

# Load the climatological SST field, lat, lon, etc.
nc_clim = Dataset(nc_clim_path)
lat_clim = nc_clim.variables[lat_clim_name]
lon_clim = nc_clim.variables[lon_clim_name]
time_clim = nc_clim.variables[time_clim_name]
sst_clim = nc_clim.variables[sst_clim_name]
num_records_clim = nc_clim.variables[num_records_clim_name]
year_clim = nc_clim.variables[year_clim_name]
month_clim = nc_clim.variables[month_clim_name]
day_clim = nc_clim.variables[day_clim_name]
try:
    T1_clim = nc_clim.variables[T1_clim_name]
    T2_clim = nc_clim.variables[T2_clim_name]
    DT_clim = nc_clim.variables[DT_clim_name]
...
for t in range(12):
    sst_anom_spline = RectBivariateSpline(lat_cont, lon_cont_pivoted,
                                          sst_anom_out[t], kx=1, ky=1)
    sst_anom_interp[t] = sst_anom_spline(lat_clim[:], lon_clim[:])

# Apply desired regional mask to the reinterpolated data.
do_mask = False
print "Do regional perturbation:", do_mask
if do_mask:
    region_name = 'wpwp'
    print "Perturbation region name:", region_name
    invert_mask = True          # True masks outside, False inside, the region
...
print "Convert to AM2 SST data time indexing:", do_am2_format
if model == 'am2':
    ...
    def am2_sst_format(sst, start_month=11, num_months=207):
        """Convert a 12 month climatology file into the AM2 SST input format."""
        if start_month == 0:
            offset = 0
        else:
...
if do_save_new_sst:
    ...
    nc_out_path = (nc_out_base_dir + name_out_prefix + '.' +
                   name_out_tag + '.nc').replace('..', '.')
    print "New SST data saved to:", nc_out_path
    nc_out = Dataset(nc_out_path, 'w', format=nc_clim.file_format)
